...
<section class="seccion contendor">
    <h2>Calendario de evento Arquitectura</h2>
    <?php
        try {
            require_once('includes/funciones/conexion.php');

            $sql = "SELECT `id_evento`, `nombre_evento`, `fecha_evento`, `hora_evento`, `cat_evento`, `nombre_invitado`, `apellido_invitado` "; 
            $sql .= "FROM `t_arquitectura` ";
            $sql .= "INNER JOIN `categoria_evento` ";
            $sql .= "ON t_arquitectura.id_cat_evento = categoria_evento.id_categoria ";

            $sql .= "INNER JOIN `invitados` ";
            $sql .= "ON t_arquitectura.id_inv = invitados.id_invitado ";
